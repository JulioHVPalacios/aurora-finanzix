...
export function showSubscriptionModal({ onSave, subscriptionToEdit = null }) {
  ...
  function renderModal() {
    ...
    overlay.innerHTML = `
      <div class="bottom-sheet" style="max-height: 92vh; overflow-y: auto; display: flex; flex-direction: column;">
        <div class="sheet-handle"></div>
        <div class="sheet-header" style="margin-bottom: 8px;">
        ...
        ${!subscriptionToEdit ? `
          ...
          <div style="margin-bottom: 8px;">
            <div style="position: relative; display: flex; align-items: center;">
              ...
              <input type="text" id="sub-search-input" class="input-control" value="${searchQuery}" placeholder="Buscar servicio: Sedapal, Luz del Sur, WIN, Netflix, BCP..." style="padding-left: 36px; padding-top: 8px; padding-bottom: 8px; font-size: 0.82rem; border-radius: 12px;" />
              ${searchQuery ? `
          ...
          <div style="margin-bottom: 14px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
              <span style="font-size: 0.68rem; font-weight: 700; color: #64748B; text-transform: uppercase; letter-spacing: 0.05em;">
                Empresas & Servicios (${filtered.length})
              </span>
              <span style="font-size: 0.68rem; color: var(--ink-40);">Toca para autocompletar</span>
            </div>

            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(105px, 1fr)); gap: 8px; max-height: 165px; overflow-y: auto; padding-right: 2px; border: 1px solid rgba(15, 23, 42, 0.06); border-radius: 14px; padding: 8px; background: #FAFBFD;">
              ${filtered.length === 0 ? `
